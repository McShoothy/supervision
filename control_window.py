...
def update_counts():
    """Update detection counts and FPS display from state file"""
    try:
        if os.path.exists(STATE_FILE):
            ...
            counts = current_state.get('detection_counts', {})
            ...
            # Update FPS display
            fps = current_state.get('video_fps', 0)
            dpg.set_value("fps_display", f"FPS: {fps}")
    except:
        pass

def update_video_progress():
    """Update video progress display from state file"""
    try:
        if os.path.exists(STATE_FILE):
            with open(STATE_FILE, 'r') as f:
                current_state = json.load(f)
            ...
            progress = current_state.get('video_progress', 0.0)
            time_str = current_state.get('video_time_str', "0:00 / 0:00")
            # Note: Progress bar would be added here if we had one in the UI
    except:
        pass
...
# Create the controls window with increased width for two columns
with dpg.window(label="VJ Controls", width=900, height=1000, tag="control_window"):
    dpg.add_text("VJ Control Panel", color=(255, 255, 0))
    dpg.add_separator()

    # Create two-column layout using groups
    with dpg.group(horizontal=True):
        # LEFT COLUMN
        with dpg.group(width=430):
            dpg.add_text("LEFT COLUMN", color=(100, 200, 255))
            dpg.add_separator()
        ...
        # RIGHT COLUMN
        with dpg.group(width=430):
            dpg.add_text("RIGHT COLUMN", color=(255, 200, 100))
            dpg.add_separator()
    ...
    dpg.add_separator()
    dpg.add_button(label="Exit VJ Program", callback=lambda: on_exit(), width=400)
    dpg.add_text("This window controls the video stream in the other window.")

# Setup viewport and show with increased width
dpg.create_viewport(title="VJ Controls", width=950, height=900)
dpg.setup_dearpygui()
dpg.show_viewport()
